...
struct FeatureCache
{
    ...
    {
        // const auto free_total = getGPUMemoryInfo();
        // min_available_physical_memory_ = free_total.first * (physical_memory_percent / 100.0);
    ...
    std::pair<unsigned long long,unsigned long long>  getGPUMemoryInfo()
    {
        size_t l_free = 0;
        size_t l_Total = 0;
        cudaError_t error_id = cudaMemGetInfo(&l_free, &l_Total);

        return {l_free, l_Total};
    }
     ...
     */
    std::size_t prune(const bool bPruneOnlyOne = true)
    {
        std::size_t count = 0;
        for (auto it = begin(cache_); it != end(cache_);)
        {
            ...
            {
                file_reader_.DownloadImageAll(*it->second, it->first);
    ...
    bool loadFeature(const int index)
    {
        ...
        {
            // cache_.find(index) != cache_.end() ? cache_.at(index) :
            std::shared_ptr<ImageDevice> image =  std::make_shared<ImageDevice>();
            std::cerr << "---------------------\nUploading image #" << index << " to GPU...\n";
            file_reader_.UploadImageAll(*image, index);
            // TODO(Ramadan) : upload the hash and bucket data from cpu
            cache_[index] = image;
            return true;
        }
        return false;
